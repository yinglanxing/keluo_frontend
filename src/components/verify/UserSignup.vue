<template>
    <q-form>
        ...
        ></q-input>
        <!--邮箱验证码-->
        <q-btn class="full-width" color="warning" @click="check_email"
            >验证邮箱
            <q-tooltip> 发送邮箱验证码 </q-tooltip>
        </q-btn>
        <q-input
            class="q-mt-md"
            v-model="email_code"
...
export default defineComponent({
    ...
    methods: {
        check_email() {
            if (this.form.email) {
                api.get('/api/v1/email_verify?email=' + this.form.email).then(
                    (req) => {
                        if (req.status == 200) {
                            // 以发送验证码
        ...
        submit() {
            ...
            api.post('/api/v1/signup?code=' + this.email_code, formData).then(
                (req) => {
                    if (req.status == 200) {
                        // 自动登录开启
                        if (this.auto_login) {
